...
const handleBodyMindPress = () => {
  const bodyMindLeft = Math.max(0, FREE_BODYMIND_LIMIT - bodyMindCount);

  if (isPremium) {
    runBodyMind();
  } else if (bodyMindLeft > 0) {
    runBodyMind();
  } else {
    Alert.alert(
      "Quota atteint",
      "Tes analyses BodyMind gratuites sont épuisées. Active BodyDiet Premium pour continuer sans limite."
    );
  }
};
  return (
  ...
  >
    {Math.max(0, FREE_BODYMIND_LIMIT - bodyMindCount) > 0
      ? `Il te reste ${Math.max(0, FREE_BODYMIND_LIMIT - bodyMindCount)} analyses BodyMind gratuites`
      : "Quota gratuit atteint • Active BodyDiet Premium pour continuer sans limite"}
  </Text>
)}
        {!bodyMindReply && (
  ...
  onPress={loadingAi ? undefined : handleBodyMindPress}
              style={{
                marginTop: 18,
                paddingVertical: 14,
                borderRadius: 14,
